...
curl http://testflightapp.com/api/builds.json -F file="@$ipaFileName" -F dsym="@$zipdSYMFileName" -F api_token="$testFlightAPIToken" -F team_token="$testFlightTeamToken" -F notes="This build was uploaded via the upload API." -F notify=True -F distribution_lists="" >> $logPath
